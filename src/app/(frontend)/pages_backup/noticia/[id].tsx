  ...
  const RichTextRenderer = ({ content }: { content: any }) => {
    ...
    const renderTextNode = (node: any, index: number): React.ReactNode => {
      if (node.type === 'text') {
        ...
        if (node.format & 8) className += ' strikethrough';
      ...
      if (node.type === 'link') {
        return (
          <a
            ...
            className="text-rcs-cta hover:text-rcs-cta/80 underline"
      ...
      <div className="bg-rcs-bg min-h-screen flex flex-col">
        ...
        <div className="container mx-auto px-4 py-8 flex justify-center items-center min-h-[50vh]">
          <div className="text-center">
            ...
            <p className="text-rcs-sec-400 mb-6">A notícia que você está procurando não existe ou foi removida.</p>
        ...
        <div className="flex flex-col lg:flex-row gap-6 mb-8">
          ...
          <div className="w-full lg:w-3/5 xl:w-2/3 transition-all duration-500 delay-75">
            <div className="mb-4 flex flex-wrap items-center gap-2">
              <a
                href={`/categoria/${news.categories?.[0]?.title?.toLowerCase()}`}
          ...
          <span className="text-sm text-rcs-sec-500 group-hover:text-rcs-cta-600 transition-colors">{news.populatedAuthors?.[0]?.name || 'Redator RCS'}</span>
              </a>
            </div>
          ...
          <img
            src={getMediaUrl(news.heroImage)}
            alt={news.title}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {relatedNews.slice(0, 3).map((item) => (
              <RelatedNewsCard
                ...
                category={item.categories?.[0]?.title || 'CS2'}
                title={item.title?.toUpperCase()}
                image={item.heroImage ? getMediaUrl(item.heroImage) : 'https://static.draft5.gg/news/2024/09/13124738/RED-Canids-coldzera-ESL-Pro-League-S20-2.jpg'}
